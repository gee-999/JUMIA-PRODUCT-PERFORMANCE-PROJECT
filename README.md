...
-Product-This column contained Product names.
-Current Price-it contained current selling price of the products in Kenyan shillings.
-Discount-Contained the discount percentages of the products.
-Review-Had number of  customer reviews recorded.
-Rating-contained the ratings given by customers out of the possible five.
...
-Two duplicates 
-Products that were written in mixed formats, some had Proper format and others were fully in caps.
-Several missing values in the rate and review columns.
-There were cells containing ranges in the current and old price column. 
-The column Rating was named Ratingd
...
Product names were turned into proper format using the =PROPER () function.
The cells containing range as a value were changed to a midpoint by averaging.
The column ratingd was rewritten as Rating.
The review column with the negative entries was highlighted and by use of CTRL + H we replaced the negative sign with a blank making the values appear without a negation.
The Rating column with the text “out of 5” was corrected by Ctrl+H where we removed the “out of 5” part.
...
-Price check- we used it to check if the prices were valid by subtracting the old price to the new price and returning Ok if it was fine or otherwise flagging it to be checked.
-Price category-with the use of the 1st and 3rd quartile we placed the Current prices in a properly defined threshold. Where Price Q_3 was 1669.5 and Price Q_1 was 493.
-Discount amount- we calculated the precise discount amount for each product.
-Discount check-we checked if the discount was between 0-1 and asked for a return of “OK” when it met the condition, otherwise it would return a “check the discount” message.
-Discount category- created threshold for discount where <20% represented low discount, >20% but <=40% represented medium discount, and >40% represented High discount.
-Review engagement flag-Created a well-defined threshold of 75% percentile-(Q_3) which acted as a wall between strong engagement and weak engagement.
-Rate flag- Checked if the rate column was well defined and returned OK if it was,otherwise returning ” Check the rating.”
-Rating category- Defined<3 as poor rating, <=4.5 as average, and >4.5 as Excellent otherwise returned missing or doesn’t meet these criteria.
-High discount and low rating flag- recorded the cells with High discount and low rating flag, otherwise returned “doesn’t meet criteria” or “Missing entry.”
-High discount and low review engagement- recorded the cells with High discount and low review engagement, otherwise returned “doesn’t meet criteria” or “Missing entry.”
-Many reviews and average rating- recorded the cells with Many reviews and average rating flag, otherwise returned “doesn’t meet criteria” or “Missing entry.”
-Strong review engagement and excellent rating- recorded the cells with Strong review engagement and excellent rating, otherwise returned “doesn’t meet criteria” or “Missing entry.”
...
2. Review Promotion Strategies Rather Than Relying on Discounts Alone
...
3. Investigate Low-Rated Products Before Increasing Promotion
...
4. Use High-Rated Products as Benchmarks
...
5. Avoid Using Price or Rating as Standalone Predictors of Engagement
...
## Limitations
Limitations
•	No sales or revenue data: The dataset does not contain actual sales, revenue, profit, or conversion data. Therefore, reviews are used only as a proxy for customer engagement and should not be interpreted as direct evidence of sales performance.
•	Missing ratings: 55 out of 112 products (49.1%) have no recorded rating. This limits the reliability of comparisons involving product ratings and may affect the overall interpretation of rating-based analysis.
•	No listing-age information: The data set does not indicate how long each product has been listed. Products that have been available for longer may naturally accumulate more reviews, which cannot be accounted for in this analysis.
•	Correlation does not imply causation: The correlation analyses show relationships between variables but cannot establish that discounts, prices, or ratings directly cause changes in review engagement.
•	Limited product and customer information: The dataset does not include detailed customer review comments, product quality measures, seller information, delivery performance, or other factors that may influence ratings and reviews.
•	Dataset size: The analysis is based on 112 products, so the findings may not represent the performance of all products on Jumia or the wider e-commerce market.
•	Snapshot of product performance: The data set represents the products available in the data collected and may not reflect changes in prices, discounts, ratings, or reviews over time.
